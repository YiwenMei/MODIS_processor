...
% opth : path to store the output images (e.g. C:\...\Albedo\);
...
% The processed albedo image is stored in opth. The naming convention is
%  [arf(8:end) yyyymmdd.tif] (arf(8:end) can be, e.g., BSA_vis, WSA_nir).

%% Additional Note
% Require MODISimg.m, doy2date.m, resizeimg.m, and matV2tif.m.

function alb=ALB_process(afl,arf,wkpth,opth,GIf,ors,varargin)
%% Check the input
narginchk(6,9);
ips=inputParser;
...
addRequired(ips,'opth',@(x) validateattributes(x,{'char'},{'nonempty'},mfilename,'opth',4));
addRequired(ips,'GIf',@(x) validateattributes(x,{'double'},{'nonempty'},mfilename,'GIf',5));
addRequired(ips,'ors',@(x) validateattributes(x,{'char'},{'nonempty'},mfilename,'ors',6));

addOptional(ips,'thr',.5,@(x) validateattributes(x,{'double'},{'scalar','>=',0,'<=',1},mfilename,...
    'thr',7));
...
parse(ips,afl,arf,wkpth,opth,GIf,ors,varargin{:});
thr=ips.Results.thr;
ndv=ips.Results.ndv;
...
kx=max(1,fix(GIf(3,1)/rxi));
ky=max(1,fix(GIf(3,2)/ryi));

%% Upscale the image
Tfl=[];
idn=fullfile(wkpth,['idA_r' num2str(GIf(3,1),'%i') '.mat']);
...
oun=fullfile(opth,[arf(8:end) ds '.tif']);
alb=MODISimg(Tfl,wkpth,oun,GIf,ors,'','','bilinear',pflg);
end
